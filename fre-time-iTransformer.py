...
class FrequencyFusion(nn.Module):
    def __init__(self, C, out_feature, window, n_fft, kernel_size, hop_length=None, win_length=None):
        super().__init__()
        self.C = C
        self.n_fft = n_fft
        ...
        self.window = window
        self.n_fft = n_fft
        self.freq_conv = nn.Conv1d(C * (n_fft // 2 + 1), out_feature, kernel_size)
...
class TCNBlock(nn.Module):
    def __init__(self, n_features, n_filters, filter_size, dilation, dropout_rate):
        super().__init__()
